...
dotplot(gene.pm.go, showCategory = 15) + ggtitle("Enrichment Analysis\nSignificant DE Genes Metastatic vs. Primary")+theme( plot.title=element_text(size=10,face="bold"),axis.title=element_text(size=15))
...
emapplot(gene_pm_pair, showCategory = 15, cex_label_category = 1, cex_category = 2, min_edge = 0.3) + ggtitle("Enrichment Map\nSignificant DE Genes Metastatic vs. Primary")+theme(plot.title=element_text(size=12,face="bold"),axis.title=element_text(size=30,face="bold"), legend.text = element_text(size = 15), legend.title = element_text(size = 20))
...
pheatmap(gene_pm_heat_sub, scale="row",main = "Feature Selected DE Genes, by Status\nShowing Selected Genes by All Methods",fontsize = 20, size = 35, color = inferno(25), show_rownames = TRUE, show_colnames = FALSE,cluster_cols = FALSE, cluster_rows = TRUE, breaks = breaklist, legend_labels = "Expression Z-score", annotation_col = metadata[,c("Status"), drop = FALSE],gaps_col = 65, annotation_colors = list(Status = status_col))
```
...
feature_selected_hervs_pm$Group <- retro.family.table[feature_selected_hervs_pm$Family, "group"]
```
...
group_col <- ggplot(graph.list$Group, aes(x = Class, y = Count, fill = reorder(Group, -Count))) + geom_bar(stat = "identity", color = "black") +  scale_fill_manual(values = polyc)+ xlab(NULL) + guides(fill = guide_legend(title = "HERV Family Group"))
cat_col <- ggplot(graph.list$Category, aes(x = Class, y = Count, fill = reorder(Category, -Count))) + geom_bar(stat = "identity", color = "black") +  scale_fill_manual(values = col_blind_p)+ xlab(NULL) + guides(fill = guide_legend(title = "HERV Category"))
chrom_col <- ggplot(graph.list$Chrom, aes(x = Class, y = Count, fill = reorder(Chrom, -Count))) + geom_bar(stat = "identity", color = "black") +  scale_fill_manual(values = varibow)+ xlab(NULL) + guides(fill = guide_legend(title = "HERV Chromosome Position"))
type_col <- ggplot(graph.list$TE_type, aes(x = Class, y = Count, fill = reorder(TE_type, -Count))) + geom_bar(stat = "identity", color = "black") +  scale_fill_manual(values = glas32)+ xlab(NULL) + guides(fill = guide_legend(title = "HERV Type"))
...
plot_contrast <- function(count_table, metadata,category, herv, title = "Selected Counts"){
  ...
  herv_plot <- ggplot(count_hervs,aes(x = Category, y = HERV, color = Category)) + geom_boxplot(outlier.shape = NA)+ geom_jitter(size = 1, shape=16, position=position_jitter(0.2))+ stat_summary(fun.y=mean, geom="point", shape=19, size=3, color = "red")+scale_color_manual(values = c("#E69F00", "#56B4E9"))+xlab(NULL)+ ylab(" Normalized Counts")+ggtitle(title)+theme(plot.title = element_text(size = 15, face = "bold"),axis.title=element_text(size=15,face="bold"),legend.title = element_text(size = 15,face="bold"),axis.text = element_text(size = 15),axis.text.x = element_text(angle = 60, vjust = 1, hjust=1))
  return(print(herv_plot))}

HERVH_13q33.3_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "HERVH_13q33.3", title = "HERVH_13q33.3")
MER4B_19q13.42a_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "MER4B_19q13.42a", title = "MER4B_19q13.42a")
HERV4_13q32.3_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "HERV4_13q32.3", title = "HERV4_13q32.3")
ERVLB4_8p21.3b_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "ERVLB4_8p21.3b", title = "ERVLB4_8p21.3b")
HERVI_7q33c_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "HERVI_7q33c", title = "HERVI_7q33c")
HERVL18_6q14.1_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "HERVL18_6q14.1", title = "HERVL18_6q14.1")
MER61_12p13.31b_pm <- plot_contrast(DESeq.herv.filt_norm, metadata = metadata, category = "Status", herv = "MER61_12p13.31b", title = "MER61_12p13.31b")
```
...
# Data Saving
```{r, eval = FALSE}
save(metadata, mets_selected, retro.family.table, te_annot, gene_annot,response_samples, raw_gene, raw_herv, hugo_metadata, hugo_gene, hugo_herv, make_sum, DESeq.gene.un.pca.3, DESeq.herv.un.pca.2, res_herv_pm.all, res_gene_pm.all, gene_heat_pm, herv_heat_pm, selected_genes_pm, selected_herv_pm, file = "/Users/phoebefei/Desktop/WCM/Mets Melanoma/TCGA SKCM/RData/2.Primary vs. Metastatic TCGA-SKCM.RData")
```