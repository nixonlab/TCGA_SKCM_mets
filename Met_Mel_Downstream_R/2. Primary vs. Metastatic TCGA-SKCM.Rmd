...
mix_cols <- c(polyc[1:length(unique(te_annot$Group))],glas32[1:length(unique(te_annot$Category))],col_blind_p[1:length(unique(te_annot$TE_type))],varibow[1:length(unique(te_annot$Chrom))])
herv_sort_cols <- c()
...
for(i in 1:length(org_clusters)){
  org_col[org_clusters[i]] <- glas32[i]
}
```
...
# Data Saving
```{r, eval = FALSE}
save(metadata, mets_selected, retro.family.table, te_annot, gene_annot,response_samples, raw_gene, raw_herv, hugo_metadata, hugo_gene, hugo_herv, make_sum, DESeq.gene.un.pca.3, DESeq.herv.un.pca.2, res_herv_pm.all, res_gene_pm.all, gene_heat_pm, herv_heat_pm, selected_genes_pm, selected_herv_pm, sum_te_table, abundance_sum, plot_contrast , cluster_col_g,cluster_col_h, feature_col, herv_sort_cols,org_col, respond_col, file = "/Users/phoebefei/Desktop/WCM/Mets Melanoma/TCGA SKCM/RData/2.Primary vs. Metastatic TCGA-SKCM.RData")
